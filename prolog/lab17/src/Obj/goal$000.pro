...
clauses
    lengthInner([], FinalAnswer, CurAnswer) :- FinalAnswer = CurAnswer, !.
    lengthInner([_|Tail], FinalAnswer, CurAnswer) :-
                            NextAnswer = CurAnswer + 1,
                            lengthInner(Tail, FinalAnswer, NextAnswer).                        
    length(List, Length) :- lengthInner(List, Length, 0).


    sumInner([], FinalSum, CurSum) :- FinalSum = CurSum, !.
    sumInner([Head|Tail], FinalSum, CurSum) :-
                                    NextSum = CurSum + Head,
                                    sumInner(Tail, FinalSum, NextSum).
    sum(List, Sum) :- sumInner(List, Sum, 0).

    
    oddSumInner([], FinalSum, CurSum) :- FinalSum = CurSum, !.
    ...
    oddSum(List, Sum) :- oddSumInner(List, Sum, 0).
